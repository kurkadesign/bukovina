...
function send_app_mail(string $to,string $subject,string $html,string $text='',array $attachments=[]):array{
 ...
 try{
  ...
  $headers=['From: '.MAIL_FROM_NAME.' <'.MAIL_FROM.'>','To: <'.$to.'>','Subject: =?UTF-8?B?'.base64_encode($subject).'?=','MIME-Version: 1.0','Content-Type: multipart/'.($attachments?'mixed; boundary="'.$mixed.'"':'related; boundary="'.$related.'"'),'Date: '.date(DATE_RFC2822),'Message-ID: <'.bin2hex(random_bytes(12)).'@'.($_SERVER['SERVER_NAME']??'localhost').'>'];
  $alternative='--'.$b."\r\nContent-Type: text/plain; charset=UTF-8\r\nContent-Transfer-Encoding: 8bit\r\n\r\n".$plain."\r\n".'--'.$b."\r\nContent-Type: text/html; charset=UTF-8\r\nContent-Transfer-Encoding: 8bit\r\n\r\n".$html."\r\n".'--'.$b."--\r\n";
  $relatedBody='--'.$related."\r\nContent-Type: multipart/alternative; boundary=\"".$b."\"\r\n\r\n".$alternative;
  if($logoData!==false)$relatedBody.='--'.$related."\r\nContent-Type: image/png; name=\"bukovina.png\"\r\nContent-Transfer-Encoding: base64\r\nContent-ID: <bukovina-logo>\r\nContent-Disposition: inline; filename=\"bukovina.png\"\r\n\r\n".chunk_split(base64_encode($logoData),76,"\r\n");
  $relatedBody.='--'.$related."--\r\n";
  $body=implode("\r\n",$headers)."\r\n\r\n";
  if($attachments){
   $body.='--'.$mixed."\r\nContent-Type: multipart/related; boundary=\"".$related."\"\r\n\r\n".$relatedBody;
   foreach($attachments as $attachment){$data=(string)($attachment['data']??'');if($data==='')continue;$name=preg_replace('/[^a-zA-Z0-9._-]/','-',(string)($attachment['name']??'attachment.bin'))?:'attachment.bin';$type=(string)($attachment['type']??'application/octet-stream');$body.='--'.$mixed."\r\nContent-Type: ".$type.'; name="'.$name."\"\r\nContent-Transfer-Encoding: base64\r\nContent-Disposition: attachment; filename=\"".$name."\"\r\n\r\n".chunk_split(base64_encode($data),76,"\r\n");}
   $body.='--'.$mixed."--\r\n";
  }else $body.=$relatedBody;
  $payload=str_replace("\r\n.","\r\n..",$body).".\r\n";
...
function email_layout(string $title,string $content,string $buttonLabel='',string $buttonUrl='',string $signature=''):string{$button=$buttonUrl!==''&&$buttonLabel!==''?'<p style="margin:28px 0"><a href="'.htmlspecialchars($buttonUrl,ENT_QUOTES).'" style="display:inline-block;background:#708e8e;color:#fff;text-decoration:none;padding:13px 20px;border-radius:0;font-weight:700">'.htmlspecialchars($buttonLabel).'</a></p>':'';return '<!doctype html><html><body style="margin:0;background:#fff;font-family:Arial,sans-serif;color:#243535;text-align:left"><div style="max-width:620px;margin:0;background:#fff;padding:32px;text-align:left"><h1 style="margin:0 0 20px;font-size:25px;text-align:left">'.htmlspecialchars($title).'</h1><div style="text-align:left">'.$content.'</div>'.$button.$signature.'</div></body></html>';}
function send_from_template(string $key,string $to,array $project,string $buttonUrl='',array $extra=[],array $attachments=[]):array{require_once __DIR__.'/email_templates.php';$t=get_rendered_email_template($key,$project,$extra);return send_app_mail($to,$t['subject'],email_layout($t['title'],$t['body'],$t['buttonLabel'],$buttonUrl,$t['signature']),'',$attachments);}
function send_client_invitation(array $p,string $url,string $userEmail=''):array{return send_from_template('invitation',(string)($p['client']['email']??''),$p,$url,$userEmail!==''?['_template_user'=>$userEmail]:[]);}
function send_submission_notifications(array $p,string $adminUrl='',string $pdf='',string $pdfName='event.pdf'):array{$r=[];$attachments=$pdf!==''?[['data'=>$pdf,'name'=>$pdfName,'type'=>'application/pdf']]:[];if(ORGANIZER_EMAIL!=='')$r['organizer']=send_from_template('submission_organizer',ORGANIZER_EMAIL,$p,$adminUrl,[],$attachments);$email=(string)($p['client']['email']??'');if($email!=='')$r['client']=send_from_template('submission_client',$email,$p);return$r;}
function send_review_decision(array $p,string $decision,string $note,string $editUrl,string $userEmail=''):array{$email=(string)($p['client']['email']??'');if($email==='')return['ok'=>false,'error'=>'Klient nemá e-mail.'];$key=$decision==='approved'?'approved':'revision';return send_from_template($key,$email,$p,$decision==='approved'?'':$editUrl,['review_note'=>$note,'_template_user'=>$userEmail]);}
