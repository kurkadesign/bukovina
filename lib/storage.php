...
function ensure_storage(): void {
 ...
 if(is_dir(DATA_DIR)&&is_writable(DATA_DIR)&&!is_file($protectionFile))file_put_contents($protectionFile,"<IfModule mod_authz_core.c>\nRequire all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\nDeny from all\n</IfModule>\n",LOCK_EX);
}
...
function write_json(string $file, array $data): void { ensure_storage(); if(isset($data['access']['shareToken'])){$legacyEnabled=(bool)($data['access']['shareEnabled']??false);$legacyAllergies=(bool)($data['share']['showDiet']??false);$data['share']=array_merge(['enabled'=>$legacyEnabled,'showAllergies'=>$legacyAllergies],is_array($data['share']??null)?$data['share']:[]);unset($data['access']['shareEnabled'],$data['share']['showDiet']);}$tmp=$file.'.tmp'; if(file_put_contents($tmp,json_encode($data,JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX)===false) throw new RuntimeException('Súbor sa nepodarilo zapísať.'); if(!rename($tmp,$file)) throw new RuntimeException('Súbor sa nepodarilo uložiť.'); }
...
function find_project_by_token(string $value, string &$mode=''): ?array { foreach (all_projects() as $p) { if (hash_equals((string)($p['access']['editToken']??''),$value)) {$mode='edit';return $p;} $shareEnabled=(bool)($p['share']['enabled']??$p['access']['shareEnabled']??false);if($shareEnabled&&hash_equals((string)($p['access']['shareToken']??''),$value)) {$mode='share';return $p;} } return null; }
