...
function email_template_user_email(string $explicit=''): string {
 ...
 secure_session_start();$email=strtolower(trim((string)($_SESSION['admin']??'')));if($email!=='')return$email;
...
function render_email_signature(array $signature): string {
 ...
 $logo=htmlspecialchars(rtrim(BASE_URL,'/').'/assets/bukovina.png',ENT_QUOTES,'UTF-8');
