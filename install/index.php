...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($action==='account'){
  ...
  elseif($password!==$confirm)$error='Heslá sa nezhodujú.';
   ...
   $_SESSION['install_admin']=['name'=>$name,'email'=>$email,'passwordHash'=>password_hash($password,PASSWORD_DEFAULT)];
...
?><!doctype html><html lang="sk"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Inštalácia – Bukovina Planner</title><style>*{box-sizing:border-box}body{margin:0;background:#f5f7f7;color:#243535;font:15px system-ui;min-height:100vh;display:grid;place-items:center;padding:24px}.wrap{width:min(760px,100%);background:#fff;padding:30px;box-shadow:0 18px 60px #20353518}h1{margin:0 0 6px;font-size:32px}h2{margin:26px 0 6px}p{color:#718181}.steps{color:#785265;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}.checks{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:24px 0}.check{display:flex;justify-content:space-between;padding:12px;background:#f5f8f8}.ok{color:#2e7d4f;font-weight:700}.bad{color:#a33b3b;font-weight:700}.warn{color:#9a6a1d;font-weight:700}form{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:24px}label{display:grid;gap:6px;font-weight:650}.full{grid-column:1/-1}input,select{padding:10px 12px;border:1px solid #ccd8d8;border-radius:0;font:inherit;background:#fff}button{grid-column:1/-1;justify-self:start;border:0;border-radius:0;min-height:36px;padding:8px 10px;background:#708e8e;color:#fff;font-weight:700;cursor:pointer}.error{padding:13px;background:#f7e5e5;color:#7c3434;margin:15px 0}small{color:#7a8888}@media(max-width:650px){.checks,form{grid-template-columns:1fr}.full{grid-column:auto}}</style></head><body><main class="wrap"><div class="steps">Krok <?=$step==='smtp'?'2 z 2':'1 z 2'?></div><h1>Bukovina Planner</h1><?php if($step==='account'):?><p>Prvotné nastavenie serverového systému.</p><section class="checks"><div class="check"><span>PHP <?=h(PHP_VERSION)?></span><b class="<?=$checks['php']?'ok':'bad'?>"><?=$checks['php']?'Vyhovuje':'Potrebné 8.1+'?></b></div><div class="check"><span>Priečinok data</span><b class="<?=$checks['data']?'ok':'bad'?>"><?=$checks['data']?'Zapisovateľný':'Bez zápisu'?></b></div><div class="check"><span>Projekty a verzie</span><b class="<?=($checks['projects']&&$checks['versions'])?'ok':'bad'?>"><?=($checks['projects']&&$checks['versions'])?'Zapisovateľné':'Bez zápisu'?></b></div><div class="check"><span>HTTPS</span><b class="<?=$checks['https']?'ok':'warn'?>"><?=$checks['https']?'Aktívne':'Odporúčané'?></b></div></section><?php if($error):?><div class="error"><?=h($error)?></div><?php endif?><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="account"><label class="full">Meno správcu<input name="name" required autocomplete="name" value="<?=h((string)($_POST['name']??''))?>"></label><label class="full">Prihlasovací e-mail<input type="email" name="email" required autocomplete="username" value="<?=h((string)($_POST['email']??''))?>"></label><label>Heslo<input type="password" name="password" minlength="12" required autocomplete="new-password"></label><label>Zopakovať heslo<input type="password" name="confirmPassword" minlength="12" required autocomplete="new-password"></label><button <?=$requiredOk?'':'disabled'?>>Ďalej</button></form><?php else:?><h2>Nastavenie odosielania e-mailov</h2><p>Zadajte údaje SMTP servera a adresy používané systémom.</p><?php if($error):?><div class="error"><?=h($error)?></div><?php endif?><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="smtp"><label class="full">Verejná URL webu<input type="url" name="baseUrl" required value="<?=h((string)($_POST['baseUrl']??$defaultBaseUrl))?>"></label><label>SMTP server<input name="smtpHost" required value="<?=h((string)($_POST['smtpHost']??'smtp.gmail.com'))?>"></label><label>SMTP port<input type="number" name="smtpPort" min="1" max="65535" required value="<?=h((string)($_POST['smtpPort']??'465'))?>"></label><label>Šifrovanie<select name="smtpEncryption"><option value="ssl" <?=(($_POST['smtpEncryption']??'ssl')==='ssl')?'selected':''?>>SSL</option><option value="tls" <?=(($_POST['smtpEncryption']??'')==='tls')?'selected':''?>>TLS</option><option value="none" <?=(($_POST['smtpEncryption']??'')==='none')?'selected':''?>>Bez šifrovania</option></select></label><label>SMTP používateľ<input name="smtpUser" required autocomplete="username" value="<?=h((string)($_POST['smtpUser']??''))?>"></label><label class="full">SMTP heslo<input type="password" name="smtpPassword" required autocomplete="new-password"></label><label>E-mail odosielateľa<input type="email" name="mailFrom" required value="<?=h((string)($_POST['mailFrom']??''))?>"></label><label>Názov odosielateľa<input name="mailFromName" required value="<?=h((string)($_POST['mailFromName']??'Svadobná sála'))?>"></label><label class="full">E-mail organizátora<input type="email" name="organizerEmail" required value="<?=h((string)($_POST['organizerEmail']??''))?>"></label><small class="full">Po dokončení sa nastavenie uloží do chráneného priečinka data a inštalátor sa automaticky uzamkne.</small><button>Dokončiť inštaláciu</button></form><?php endif?></main></body></html>
