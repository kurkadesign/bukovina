...
<main class="wrap">
 ...
 <form method="post">
  ...
  <label>SMTP server<input name="smtpHost" required value="<?=h((string)($_POST['smtpHost']??''))?>"></label>
  <label>SMTP port<input type="number" name="smtpPort" min="1" max="65535" required value="<?=h((string)($_POST['smtpPort']??'587'))?>"></label>
  <label>Šifrovanie<select name="smtpEncryption"><option value="ssl" <?=(($_POST['smtpEncryption']??'')==='ssl')?'selected':''?>>SSL</option><option value="tls" <?=(($_POST['smtpEncryption']??'tls')==='tls')?'selected':''?>>TLS</option><option value="none" <?=(($_POST['smtpEncryption']??'')==='none')?'selected':''?>>Bez šifrovania</option></select></label>
  ...
  <label>Názov odosielateľa<input name="mailFromName" required value="<?=h((string)($_POST['mailFromName']??'Bukovina Košice'))?>"></label>
