...
export const TYPES=[
  ...
  ['plant','Kvety','fa_flower_tulip',70,70,0,{icon:true,text:true,shape:'none',iconColor:'var(--plant-color)'}],
...
function addEventRuntimeAlias(state){Object.defineProperty(state,'wedding',{configurable:true,enumerable:false,get(){return this.event},set(value){this.event=value}});return state}
export function initialState(){const t1=uid(),t2=uid(),t3=uid(),g1=uid(),g2=uid(),g3=uid(),g4=uid();return addEventRuntimeAlias({schemaVersion:1,projectId:uid(),event:{eventName:'Event J a M',date:'2026-09-12',contactName:'Jana',phone:'',email:'',note:''},settings:{zoom:0.72,panX:60,panY:35},items:[
  ...
  {id:uid(),type:'dj',name:'DJ',x:90,y:450,width:150,height:85,rotation:0,number:1,seats:0,note:'',locked:false,defaultKey:'dj-1'},{id:uid(),type:'bar',name:'Bar',x:1450,y:880,width:190,height:75,rotation:0,number:1,seats:0,note:'',locked:false,defaultKey:'bar-1'},{id:uid(),type:'dance-floor',name:'Tanečný parket',x:1100,y:350,width:360,height:300,rotation:0,number:1,seats:0,note:'',locked:false,defaultKey:'tanecny-parket-1'}],guests:[
...
function povodnyPredvolenyKluc(item){const kluce={'head-table:0':'hlavny-stol','round-table:1':'okruhly-stol-1','round-table:2':'okruhly-stol-2','round-table:3':'okruhly-stol-3','dj:1':'dj-1','bar:1':'bar-1','dance-floor:1':'tanecny-parket-1'};return kluce[`${item.type}:${Number(item.number)||0}`]||''}
export function load(){try{const storageKey=String(globalThis.__EVENT_STORAGE_KEY__||'event-planner-v1'),raw=localStorage.getItem(storageKey);if(!raw)return initialState();const s=addEventRuntimeAlias(JSON.parse(raw));if(!s.items||!s.guests||!s.event)throw Error();if(!s.event.eventName&&s.event.name)s.event.eventName=s.event.name;delete s.event.name;delete s.event.venue;s.guests.forEach(g=>delete g.childAge);const pouzite=new Set(s.items.map(i=>i.defaultKey).filter(Boolean));s.items.forEach(i=>{if(i.defaultKey===undefined){const kluc=povodnyPredvolenyKluc(i);i.defaultKey=kluc&&!pouzite.has(kluc)?kluc:'';if(i.defaultKey)pouzite.add(i.defaultKey)}});return s}catch{return initialState()}}
export function validProject(s){return !!(s&&s.schemaVersion===1&&Array.isArray(s.items)&&Array.isArray(s.guests)&&s.event&&s.settings)}
