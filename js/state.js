...
export function load(){try{const storageKey=String(globalThis.__EVENT_STORAGE_KEY__||'event-planner-v1'),raw=localStorage.getItem(storageKey);if(!raw)return initialState();const s=JSON.parse(raw);if(!s.items||!s.guests||!s.wedding)throw Error();if(!s.wedding.eventName&&s.wedding.name)s.wedding.eventName=s.wedding.name;delete s.wedding.name;delete s.wedding.venue;s.guests.forEach(g=>delete g.childAge);const pouzite=new Set(s.items.map(i=>i.defaultKey).filter(Boolean));s.items.forEach(i=>{if(i.defaultKey===undefined){const kluc=povodnyPredvolenyKluc(i);i.defaultKey=kluc&&!pouzite.has(kluc)?kluc:'';if(i.defaultKey)pouzite.add(i.defaultKey)}});return s}catch{return initialState()}}
